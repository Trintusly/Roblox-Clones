...
";
?>
<img id='avatar' class='Avatar' src='../../images/<?php echo"" . $user->id . ""; ?>.png?r=<?php echo "$RefreshRate"; ?>' height='350' width='350' onerror="this.src='images/default.png'" />
<?php
echo "</center>
";
...
echo '<div style="height: 15px;"></div>';
echo"
<font style='text-align: center;padding:15px;'>" . $user->description . "</font>
";
...
if ($ban) {
    ...
    $conn->query("UPDATE users SET banned='1' WHERE id='$user->id'");
    $conn->query("INSERT INTO bans (banned_user, reason, ban_time, banned_by)
VALUES ('$id','$reason','$banTime','$user->id')");
	
echo"$user->username has been banned! Refresh to see.";
	}
if ($user->id != $user->id) {
if($user->power >=1){
...
if($loggedIn){
    if ($user->id != $user->id) {
        ...
        echo"<a href='https://www.tetrimus.xyz/chat/$fetchuser->username'><button name='#' type='submit' style='margin-left: 10px;' class='btn btn-primary'>Send Message</button>"; 
