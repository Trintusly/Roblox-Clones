...
<meta name="description" content="<?php echo "".$getthread->message.""; ?> | Tetrimus</title>">
...
<div class="container">
  <div class="col-md-10" style="margin: 0px auto;float: none;">
    ...
    function breadcrumbs($sep = '', $home = 'Home') {
        ...
        $last_piece = end($crumbs);
            ...
            if ($last_piece!==$crumb){
            //Make the next crumb
            $bc     .= '<li><a href="'.$site.'">'.$link.'</a>'.$sp.'</li>';
            } else {
  ...
  Delete [NEW]
</button><form method="POST" action=""><button type="submit" name="delete" class="btn btn-outline-danger" style="float:right;margin-left: 10px; data-toggle="modal" data-target="#myModal2">Delete thread</button></button>';
    }
...
<div style="width:100%;">
<div style="border-radius:5px;padding:10px;background-color:#007bff;border:1px solid #007bff;color:#fff;margin:0 auto;">
...
if (time() > $OriginalPoster->expireTime) {
    echo "<i style='color:#eaedeb;border-radius: 10px;width: 12px;height: 12px;' class='fa fa-circle' aria-hidden='true' data-toggle='tooltip' data-placement='top' title='" . $cleanuser . " is offline'></i>";
      ...
      echo"<a href='../profile.php?id=$OriginalPoster->id' style='margin-left: 7px;color:$OriginalPoster->name_color;";if($OriginalPoster->donator == 0){echo "color: #4d89ea;";}echo"'>$OriginalPoster->username<br><img class='img-fluid' style='width:200px;' src='../images/" . $OriginalPoster->id . ".png?r=$RefreshRate'"?> onerror="this.src='https://www.tetrimus.xyz/images/default.png'"<?php echo"</a><br>
...
echo "
    ...
    <div style='padding-top:10px;color:#323a46;word-break:break-word;font-size:16px;'><div style='white-space: pre-wrap'>";echo htmlentities(filter($OriginalPost->message));echo "</div>
";
if($OriginalPoster->signature && $OriginalPoster->signature != "none") {
  echo "
<div style='padding-top:10px;color:#323a46;word-break:break-word;font-size:16px;background-color: grey;border: darkgrey;border: solid;border-width: 20px;font: white;font-color: white;color: white;border: red;border: solid;border-color: darkgrey;padding-top: 2px;'>Signature: <br> <div style='white-space: pre-wrap'>$OriginalPoster->signature</div></div>
  ";
}
echo "
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <button type="submit" class="btn btn-danger" name="delete">Delete thread</button>
...
if (time() > $repliesPoster->expireTime) {
    echo "<i style='color:#eaedeb;border-radius: 10px;width: 12px;height: 12px;margin-left: 7px;' class='fa fa-circle' aria-hidden='true' data-toggle='tooltip' data-placement='top' title='" . $cleanuser . " is offline'></i>";
      ...
      echo "<a href='../profile.php?id=$repliesPoster->id' style='margin-left: 7px;color:$repliesPoster->name_color;";if($repliesPoster->donator == 0){echo "color: #4d89ea;";}echo"'>$repliesPoster->username<br><img style='width:200px;' class='img-fluid' src='../images/" . $repliesPoster->id . ".png?r=$RefreshRate'"?>onerror="this.src='https://www.tetrimus.xyz/images/default.png'"<?php echo"</a><br>
";
...
<div style='color:#9c9c9c;font-size:14px;'><i class='fa fa-clock-o' aria-hidden='true'></i> Posted at ".date('m/d/y', $output['date'])."<a href=''><i class='fa fa-flag-o' style='float:right;' aria-hidden='true'></i><a/>&nbsp;&nbsp;
    ...
    ";
    if($repliesPoster->signature && $repliesPoster->signature != "none") {
      echo "
      <hr>
<div style='padding-top:10px;color:#323a46;word-break:break-word;font-size:16px;background-color: grey;border: darkgrey;border: solid;border-width: 20px;font: white;font-color: white;color: white;border: red;border: solid;border-color: darkgrey;padding-top: 2px;'>Signature: <br> <div style='white-space: pre-wrap'>$repliesPoster->signature</div></div>";
}
    if($user->power >1){
