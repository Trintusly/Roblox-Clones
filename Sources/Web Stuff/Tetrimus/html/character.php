...
<center><div class="alert alert-primary" role="alert">
  Customizing is undergoing maintenance. Check the status at: <a href="https://www.tetrimus.xyz/discord">Our Discord</a>
</div></center>
...
<script>
      function render() {
        ...
        $('#avatar').html( '<img src="https://www.tetrimus.xyz/images/loading.gif" style="width:300px;height:300px;border-top: 30px solid #fff;border-bottom: 30px solid #fff;">' );
        
        $.get ('render.php', function ( result ) {
            
            if (result != "false") {
                document.getElementById("avatar").innerHTML = '<img src="https://www.tetrimus.xyz/images/' + result + '" style="width:300px">';
            } else {
                $.get ('render.php', function ( result ) {
                    document.getElementById("avatar").innerHTML = '<h4>There seems to be an issue! Please try again!</h4><img src="https://tetrimus.xyz/images/'+result+'.png" style="width:300px">';
                });
            }
...
<script>
setTimeout(function() {
                      document.getElementById("avatar").setAttribute("src", "https://tetrimus.xyz/images/' . $user->id . '.png?r=' . $rand . '");
                  }, 800);
</script>
';
echo "
<center>
<div id='avatar'><img src='https://tetrimus.xyz/images/loading.gif?r=$rand' style='width:300px'></div>
<br>
</center>
";
?>
...
<?php echo'
<button class="btn btn-outline-danger btn-sm" name="purge" value="purge" onclick="purge()">Purge</button>';
function purge(){
    $conn->query("UPDATE users SET Hat='none' WHERE id='$user->id'");
$conn->query("UPDATE users SET Hat2='none' WHERE id='$user->id'");
 }
...
<div class='col-md-3'>
        ...
        <div class='card-body'>
         <a href='../store/item.php?id=$storeitem->id'>
            <img style='width: 130px;height: 130px;margin-top: 10px;' src='https://www.tetrimus.xyz/images/s" . $storeitem->id . ".png?r=8' alt='$storeitem->name'>
          </a>";
