<?php
	...
	$serverName = ($_SERVER['HTTP_HOST'] == 'brickcreate.com' || $_SERVER['HTTP_HOST'] == 'brickcreate.com' || $_SERVER['HTTP_HOST'] == 'asudhsa.troplo.host' || $_SERVER['HTTP_HOST'] == 'cdn.brickcreate.com') ? 'http://'.$_SERVER['HTTP_HOST'] : NULL;
	if (!$serverName) { header('Location: https://brickcreate.com'); die; }
	...
	if (true) {
		...
		$a = $SiteSettings->fetch(PDO::FETCH_OBJ);
		$SiteSettings = $a;
		//$cache->set('SiteSettings', $a, 86400);
	...
	if ($SiteSettings->Maintenance ) {
		if (!$wearebuilding) {
			if ($_SERVER['REQUEST_URI'] != "/we-are-building/") {
	...
	function linkify($string) {
		...
		$rexProtocol = '(https?://)?';
		$rexDomain   = '(www.brickplanet.me|brickplanet.me|jobs.brickcreate.com|helpme.brickcreate.com|www.brickcreate.com|brickcreate.com|blog.brickcreate.com)';
