...
require_once($_SERVER['DOCUMENT_ROOT']."/../private/header.php");
			...
			if ($modify_file->uploaded) {
				...
				if ($modify_file->image_src_x != 887 && $modify_file->image_src_y != 887) {
					...
					if ($modify_file->processed) {
						...
						$Insert = $db->prepare("INSERT INTO Item (ItemType, Name, Description, CreatorID, CreatorType, TimeCreated, TimeUpdated, Cost, SaleActive, PreviewImage, BackendFile) VALUES(6, ?, ?, ".$gG->ID.", 1, ".time().", ".time().", ?, ?, '".$PreviewImage."', '".$BackendFile."')");
	...
	echo '
	<script>
	document.title = "Create Group Pants - Brick Create";
