...
require_once($_SERVER['DOCUMENT_ROOT']."/../private/header.php");
	...
	echo '
	<script>document.title = "My Posts - Brick Create";</script>
	<div class="grid-x grid-margin-x">
		<div class="large-9 cell">
			...
			(SELECT User.ID, User.Username, User.AvatarURL, ForumThread.ID, ForumThread.Title, ForumThread.Post, ForumThread.TimePost, ForumThread.TimeUpdated, ForumThread.Locked, ForumThread.Pinned, ForumThread.Views, ForumThread.Replies, ForumThread.LastPostReplyID, (SELECT User.Username FROM ForumReply JOIN User ON ForumReply.UserID = User.ID WHERE ForumReply.ID = ForumThread.LastPostReplyID) AS LastPostReplyPoster FROM ForumThread JOIN User ON ForumThread.UserID = User.ID WHERE ForumThread.UserID = ".$myU->ID.")
			UNION (SELECT User.ID, User.Username, User.AvatarURL, ForumThread.ID, ForumThread.Title, ForumThread.Post, ForumThread.TimePost, ForumThread.TimeUpdated, ForumThread.Locked, ForumThread.Pinned, ForumThread.Views, ForumThread.Replies, ForumThread.LastPostReplyID, (SELECT User.Username FROM ForumReply JOIN User ON ForumReply.UserID = User.ID WHERE ForumReply.ID = ForumThread.LastPostReplyID) AS LastPostReplyPoster FROM ForumThread JOIN ForumReply ON ForumThread.ID = ForumReply.ThreadID JOIN User ON ForumThread.UserID = User.ID WHERE ForumReply.UserID = ".$myU->ID.")
			ORDER BY Pinned DESC, TimeUpdated DESC
			");
			$getThreads->execute();
			...
			if ($getThreads->rowCount() == 0) {
				...
				<div class="container border-wh md-padding">
					<div class="text-center">
						<i class="material-icons" style="font-size:50px;">message</i>
						<div style="font-size:18px;">You haven\'t posted anything on the forum yet.</div>
					</div>
				</div>
				';
