...
require_once($_SERVER['DOCUMENT_ROOT']."/../private/header.php");
	...
	<meta property="og:image" content="https://cdn.brickcreate.com/8ca17bec-0320-4293-90e5-dfc5b8690156-thumb.png">
	<meta property="og:site_name" content="Brick Planet">
	<meta property="og:description" content="'.$gU->Username.' is a player on Brick Planet, a user generated content sandbox game with tens of thousands of active players. Play today!">
	<script src="https://twemoji.maxcdn.com/2/twemoji.min.js"></script>
	<script>
		document.title = "'.$gU->Username.' - Brick Planet";
	...
	<div class="grid-x grid-margin-x">
		<div class="profile-left medium-5 cell">
			...
			echo '
				<img src="https://cdn.brickcreate.com/975289c5-1c07-463c-bcf5-0b49d35c934hn.png" class="avatar-profile">
			</div>
			';
			...
			<div class="container border-r md-padding">
				...
				if ($gU->VIP == 1) {
					...
					echo '
					<div class="profile-info-line">
						<img src="https://cdn.brickcreate.com/assets/images/profile/diamond.png" width="20">
						<span style="color:#02caf2;font-weight:700;">Diamond Membership</span>
					</div>
					';
					...
					echo '
					<div class="profile-info-line">
						<img src="https://cdn.brickcreate.com/assets/images/profile/platinum.png" width="20">
						<span style="color:#02caf2;font-weight:600;">Diamond Membership+</span>
					</div>
					';
					...
					foreach ($UserProfileCache['friends']['data'] as $gF) {
						...
						<div class="large-4 medium-4 small-4 cell profile-friend text-center">
							<a href="'.$serverName.'/users/'.$gF->Username.'/">
								<div class="profile-friend-preview relative" style="background-image:url(https://cdn.brickcreate.com/8ca17bec-0320-4293-90e5-dfc5b8690156-thumb.png);background-size:cover;">
								'.$StatusSpan.'
								</div>
							</a>
											...
											if ($AUTH) {
												...
												function playGame() {
													$.get("'.$serverName.'/API/Engine/generateGameAuthToken.php?gameId='.$gG->ID.'", function(res, status) {
														var token = res.token;
														window.location.assign("brickplanet:Client_" + token);
													});
												}
												</script>
												';
												...
												if ($gU->ID == $myU->ID) {
													...
													function editWorkshop() {
														$.get("'.$serverName.'/API/Engine/generateGameAuthToken.php?gameId='.$gG->ID.'", function(res, status) {
															var token = res.token;
															window.location.assign("brickplanet:Workshop_" + token);
														});
													}
													</script>
