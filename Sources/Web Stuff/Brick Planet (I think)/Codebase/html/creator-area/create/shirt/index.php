...
require_once($_SERVER['DOCUMENT_ROOT']."/../private/header.php");
		...
		if (strlen($_POST['name']) < 3 || strlen($_POST['name']) > 70) {
			...
			if ($modify_file->uploaded) {
				...
				if ($modify_file->image_src_x != 887 && $modify_file->image_src_y != 887) {
					...
					$errorMessage = 'We\'re sorry, this type of template is not allowed. It must have dimensions of 887px by 887px.';
					...
					if ($modify_file->processed) {
						...
						if ($CountItem->fetchColumn() > 0) {
							...
							$Insert = $db->prepare("INSERT INTO Item (ItemType, Name, Description, CreatorID, TimeCreated, TimeUpdated, Cost, SaleActive, PreviewImage, BackendFile) VALUES(5, ?, ?, ".$myU->ID.", ".time().", ".time().", ?, ?, '".$PreviewImage."', '".$BackendFile."')");
							...
							$InsertUserActionLog->bindValue(1, 'Created a shirt (Item ID: '.$ID.')', PDO::PARAM_STR);
	...
	echo '
	<script>
	document.title = "Create Shirt - Brick Create";
