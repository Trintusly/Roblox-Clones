...
function openNotification(Id, url) {
	var xhr = new XMLHttpRequest();
	xhr.open("POST", "https://www.brickcreate.com/inbox/view-notification/" + Id, true);
...
function fetchUnreadUserNotifications() {
	...
	var xhr = new XMLHttpRequest();
	xhr.open("GET", "https://www.brickcreate.com/api/user/get-unread-notifications", true);
	xhr.onload = function (e) {
		if (xhr.readyState === 4) {
			if (xhr.status === 200) {
					...
					for (var i = 0; i < json.content.length; i++) {
						...
						<div class="notification-row" onclick="openNotification(` + row.notificationId + `, \'` + row.notificationUrl + `\')">
							<div class="grid-x grid-margin-x align-middle">
								...
								<div class="auto cell">
									<span><a href="https://www.brickcreate.com/users/` + row.senderUsername + `/"><strong>` + row.senderUsername + `</strong></a></span>
									<span>&nbsp;&bullet;&nbsp;</span>
