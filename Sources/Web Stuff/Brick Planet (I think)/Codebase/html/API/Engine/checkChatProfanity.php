...
require_once($_SERVER['DOCUMENT_ROOT']."/../private/memcached.php");
	...
	function isProfanity($UserId, $Text) {
	    ...
	    if ($Flag == 1) {
	        ...
	        $Insert = $db->prepare("INSERT INTO ap.ProfanityFilterLog (UserID, UserIP, TextContent, TimeLog, BadWords) VALUES(".$UserId.", '".$UserIP."', ?, ".time().", ?)");
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST' && $Username) {
		...
		$GetUser = $db->prepare("SELECT ID FROM User WHERE User WHERE Username = ?");
		$GetUser->bindValue(1, $Username, PDO::PARAM_STR);
		$GetUser->execute();
		$UserId = $GetUser->fetchColumn();
	
		$Determination = (bool)(int)isProfanity((int)$UserId, $_POST['chat']);
		
		echo json_encode(array('status' => 'ok', 'isProfane' => $Determination));
	
	} else {
		
		echo json_encode(array('status' => 'error', 'msg' => 'Invalid credential(s).'));
