<?php
	include "Header.php";
		...
		$counter = 0;
			$getDrafts = mysql_query("SELECT * FROM 3DItemDrafts");
			...
			while ($gD = mysql_fetch_object($getDrafts)) {
				...
				<center>
					...
					<img src='../3DStores/Dir/".$gD->File."' width='100' height='200'></div>
					";
					}
					else {
					echo "
					<div style='width:100px;height:200px;background-image:url(../3DStores/Dir/".$gD->File.");'>
					<img src='../3DStores/Dir/Avatar.png' width='100' height='200'>
					</div>
					";
					}
					echo "
				...
				if ($Submit) {
					...
					mysql_query("UPDATE 3DItemDrafts SET Name='$Name' WHERE ID='$ItemID'");
					mysql_query("UPDATE 3DItemDrafts SET Price='$Price' WHERE ID='$ItemID'");
					mysql_query("UPDATE 3DItemDrafts SET saletype='$saletype' WHERE ID='$ItemID'");
					mysql_query("UPDATE 3DItemDrafts SET numberstock='$numberstock' WHERE ID='$ItemID'");
					header("Location: ItemRelease.php");
				
				}
				if ($Release == "yes") {
					
					$getItem = mysql_query("SELECT * FROM 3DItemDrafts WHERE ID='$ReleaseID'");
					...
					mysql_query("INSERT INTO 3DItems (Name, File, Type, Price, CreatorID, saletype, numbersales, numberstock, sell, Description, CreationTime, store, timemake, itemDeleted, SalePrices, NumberSold)
					...
					mysql_query("DELETE FROM 3DItemDrafts WHERE ID='$ReleaseID'");
					header("Location: ItemRelease.php");					
