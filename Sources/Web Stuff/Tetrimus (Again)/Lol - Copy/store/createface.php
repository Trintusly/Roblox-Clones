...
if(isset($_POST['submit'])){
	...
	$createitemquery = "INSERT INTO `store`(name, creator_id, description, file, type, payment, price, rare, quantity, sale, accepted, object, texture, sales) VALUES('$name','$user->id','$description','tetcap','faces','tokens','$tokens','0','0','1','0','','$imgName','0')";
	$new = $conn->query($createitemquery);
    $item_id = $conn->insert_id;
    move_uploaded_file($imgTmp,"../storage/store_storage/thumbnails/".$item_id.".png");
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <select class="form-control" style="width: 15%;" id="exampleFormControlSelect1">
                        ...
                        <?php
                            if($user->power >= 3){
                                echo '<option name="hat">Hat</option>';
                                echo '<option name="face">Face</option>';
                            }
                        ?>
                    </select>
                    <div style="height: 10px"></div>
