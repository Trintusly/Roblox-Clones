...
<table width='80%'>
		...
		<td style='width:250px;' valign='top'>
			<div id='ProfileText'> 
				...
				<div id=''>
					<center>
					";
					echo "<img src='https://brickplanet.gq/Avatar.php?ID=$myU->ID'>";
					echo "
				</div>
			<br />
			<div id='ProfileText'>
			...
			<div style='overflow-x:auto;width:348px;max-width:348px;'>
				...
				if (!empty($myU->Background)) {
						$counter++;
						$checkifitem = mysql_query("SELECT * FROM Items WHERE File='".$myU->Background."'");
						$check1 = mysql_num_rows($checkifitem);
				
					if ($check1 == "1") {
					
						$getName = mysql_query("SELECT * FROM Items WHERE File='$myU->Background'");
	...
	$TheItemName = $TheItemName . "...";
					...
					<td valign='top' align='center'>
						...
						$echo1<img src='https://brickplanet.gq/Store/Dir/".$gI->File."'>
						<br />
							...
							if($Page&&$Sess&&$Var&&$Wear)
							{
								...
								else if($gi1 >= 1)
								{
									...
									if($gi->code1 == $code1&&$gi->code2 = $code2)
									{
										...
										header("Location: https://brickplanet.gq/character.php?Page=$Page");
					...
					";
								...
								if (!empty($myU->Eyes)) {
						$counter++;
						$checkifitem = mysql_query("SELECT * FROM Items WHERE File='".$myU->Eyes."'");
						$check1 = mysql_num_rows($checkifitem);
				
					if ($check1 == "1") {
					
						$getName = mysql_query("SELECT * FROM Items WHERE File='$myU->Eyes'");
						$gN = mysql_fetch_object($getName);
	...
	$TheItemName = $TheItemName . "...";
					...
					";
								...
								if (!empty($myU->Top)) {
						$counter++;
						$checkifitem = mysql_query("SELECT * FROM Items WHERE File='".$myU->Top."'");
						$check1 = mysql_num_rows($checkifitem);
				
					if ($check1 == "1") {
					
						$getName = mysql_query("SELECT * FROM Items WHERE File='$myU->Top'");
						$gN = mysql_fetch_object($getName);
	...
	$TheItemName = $TheItemName . "...";
					...
					";
								...
								if (!empty($myU->Hat)) {
						$counter++;
						$checkifitem = mysql_query("SELECT * FROM Items WHERE File='".$myU->Hat."'");
						$check1 = mysql_num_rows($checkifitem);
...
if ($Items == 0) {
echo "Looks like you don't have any items! Why not look for some in the <a href='https://brickplanet.gq/Store/UserStore.php'>Store</a>?";
}
...
if ($Page > 1) {
echo '<a href="https://brickplanet.gq/character.php?Page='.($Page-1).'">Previous</a> - ';
}
echo '   Page '.$Page.' of '.(ceil($num / $Setting["PerPage"]));
if ($Page < ($amount)) {
echo ' - <a href="https://brickplanet.gq/character.php?Page='.($Page+1).'">Next</a>';
}
