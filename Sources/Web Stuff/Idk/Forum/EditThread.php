...
$getItems = mysql_query("SELECT * FROM Threads WHERE ID='$ID'");
...
$gC = mysql_fetch_object($getcheck);
	...
	if($myU->PowerAdmin == "true"||$myU->PowerMegaModerator == "true"||$gI->CreatorID == $myU->ID)
	{
		...
		<center><table width='100%'><tr><td>
						<div id='TopBar' style='width:99%;'>Edit Thread: ".$gI->Title."
			</div>
			<div id='aB'>
		<form action='' method='POST'>
						Thread Name: <textarea name='ItemName' rows='1' cols='45'>".$gI->Title."</textarea>
<br>
		...
		$ItemName = filter($ItemName);
			mysql_query("UPDATE Threads SET Title='$ItemName' WHERE ID='$ID'");
			mysql_query("UPDATE UserStore SET Name='$ItemName' WHERE ID='$ID'");
			mysql_query("UPDATE UserStore SET Price='$Price' WHERE ID='$ID'");
			mysql_query("UPDATE Threads SET Body='$Description' WHERE ID='$ID'");
