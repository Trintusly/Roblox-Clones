...
$Username = mysql_real_escape_string(strip_tags(stripslashes($_GET['Username'])));$UIGN = mysql_real_escape_string(strip_tags(stripslashes($_GET['UIGN'])));
if($Username) {$getUser = mysql_query("SELECT * FROM Users WHERE Username='".$Username."'");
}if($ID) {$getUser = mysql_query("SELECT * FROM Users WHERE ID='".$ID."'");
}if($UIGN) {$getUser = mysql_query("SELECT * FROM Users WHERE ingamenum='".$UIGN."'");}
