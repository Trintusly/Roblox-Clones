...
include($_SERVER['DOCUMENT_ROOT']."/Header.php");
	...
	$getGames = mysql_query("SELECT * FROM Games WHERE ID='".$ID."' AND itemDeleted='0'");
	$gI = mysql_fetch_object($getGames);
	...
	$gC = mysql_fetch_object($getCreator);
		...
		$Exist = mysql_num_rows($getGames);
		if ($Exist == "0") {
		echo "<b>This game does not exist.</b>";
		}
		else {
		...
		<tr>
			<td>
				...
				<table width='100%'>
					<tr>
						<td width='50%'>
							<b style='text-transform:uppercase;font-size:8pt;'>Type: $gI->Type</b>
						</td>
						<td>
																		<font color='red'><b>Plays: ".number_format($gI->Plays)."</b></font> &nbsp; &nbsp; "; if ($gI->Active == "1") { if ($Status->ItemStatus == 0) { echo "<a href='../Games/Dir/".($gI->PlayLink)."'target='_blank'>Click To Install for Windows</a><br><a href='../Games/Dir/".($gI->PlayLinkMac)."'target='_blank'>Click To Install for Mac</a></form>"; } else if ($gI->Active == "0") { echo "<font color='red'>Sorry, The game is still prending moderation!. Please try again later.</font>"; } } if ($gI->saletype == "limited") { echo "<font color='red' style='padding-left:15px;'><b>$gI->numberstock/$gI->numbersales remaining</b></font>"; } else { echo "<font color='red' style='padding-left:15px;'><b>Status: ".($gI->Playable)." </b></font>"; } echo "
						</td>
					</tr>
				</table>
					<table>
						<tr>
							<td valign='top'><center>
								<div style='border-radius:5px;width:150px;height:200px;'>
									
									";
									if ($gI->saletype == "limited") {
									echo "
										
										<div style='width: 150px; height: 150px; z-index: 3;  background-image: url(Dir/".$gI->CoverFile.");'>
										<div style='width: 150px; height: 200px; z-index: 3;  background-image: url(/Imagess/LimitedWatermark1.png);'>
										</div></div>
									";
									}
									else {
										echo "<img src='Dir/".$gI->CoverFile."' width='150' height='150'>";
									}
									echo "
								</div>
							</td>
							<td valign='top'>
											...
											echo "<br /><a href='#' style='font-size:8pt;'>Edit Item</a>
											<br />
											<br />
							...
							<td valign='top' style='padding-left:50px;'><center>
											<b style='font-size:8pt;'>Creator</b><br /><a href='/user.php?ID=".$gC->ID."' border='0' style='color:black;'>
											";
							...
							while ($getS = mysql_fetch_object($getSales)) {
								...
								<table width='100%' style=''>
									<tr>
										<td width='100'><center>
											";
											echo "<a href='../user.php?ID=$getS->UserID'><img src='http://noynac.com/genvatar.php?ID=$getS->UserID' width='35'></a>";
										...
										<td width='200'>
											...
											if ($tG == 0) {
												...
												echo "</form><form action='Game.php?ID=".$ID."&SaleID=".$getS->ID."' method='POST'><input type='submit' name='PurchaseSale' value='Purchase' style='cursor:pointer;' id='buttonsmall'></form>";
											
											}
											else {
											
												echo "</form><form action='Game.php?ID=".$ID."&SaleID=".$getS->ID."' method='POST'><input type='submit' name='PurchaseSale' value='Purchase' disabled='disabled'  id='buttonsmall' title='You already own this item'></form>";
				...
				";
											if ($gI->saletype == "limited" && $gI->numberstock == "0"&&$tG >= "1" && $myU->ID != $gC->ID) {
												...
												<form action='' method='POST'>
													...
													<br />
													<a href='Game.php?ID=".$ID."&RemoveMySale=T'><b>( remove )</b></a>
													<br />
													";
			...
			<td width='60%' valign='top' >
				...
				<center><a name='Comments'></a>
					";
						...
						$getComments = mysql_query("SELECT * FROM GameComments WHERE ItemID='".$ID."' ORDER BY time DESC");
					...
					";
									...
									<td valign='top'><div align='left'>
										...
										if ($myU->PowerAdmin == "true"||$myU->PowerMegaModerator == "true") {
											...
											echo "
											<br /><br />
											<a href='Game.php?ID=$ID&CommentMethod=Scrub&CommentID=$gC->ID' style='color:blue;font-weight:bold'>Scrub</a> | <a href='Game.php?ID=$ID&CommentMethod=Delete&CommentID=$gC->ID' style='color:blue;font-weight:bold'>Delete</a>
											";
							...
							";
											...
											if ($CommentMethod == "Scrub") {
												...
												mysql_query("UPDATE GameComments SET Post='[ Content Deleted ]' WHERE ID='".$CommentID."'");
												header("Location: Game.php?ID=$ID#Comments");
											
											}
											if ($CommentMethod == "Delete") {
											
												mysql_query("DELETE FROM GameComments WHERE ID='".$CommentID."'");
												header("Location: Game.php?ID=$ID#Comments");
						...
						$SubmitReply = mysql_real_escape_string(strip_tags(stripslashes($_POST['SubmitReply'])));
							...
							if ($SubmitReply) {
								...
								if (time() >= $myU->comment_flood) {
									...
									mysql_query("INSERT INTO GameComments (UserID, ItemID, Post, time) VALUES ('".$myU->ID."','".$ID."','".$Reply."','".time()."')");
				...
				<div id='ProfileText'>
					...
					<div align='left'>
						";
							...
							$getRandom = mysql_query("SELECT * FROM Games ORDER BY RAND() LIMIT 3");
						...
						<table width='100%' cellspacing='0' cellpadding='0'>
							<tr>
								<td width='50'>
									<a href='/Game.php?ID=".$gR->ID."' border='0'>
									<img src='/Games/Dir/".$gR->File."' height='100'>
									</a>
								</td>
								<td valign='top' align='left'>
									...
									<br />
									<br />
									<b><font color='red'><b>Plays: ".($gR->Plays)."</b></font></b>
							...
							echo "
								<div style='background-image:url(/Imagess/menuhover.png);width:100%;height:100%;top:0;left:0;position:fixed;'>
									...
									<div style='background:#ccc;border:1px solid gray;width:500px;padding:8px;text-align:left;'>
										...
										<br />
										<br />
										<a href='Game.php?ID=".$ID."' style='color:black;font-weight:bold;'>Return</a>
									</div>
								</div>
							";
