...
$connection = mysql_connect("localhost","brickpla_usr","2zqKKQQ*bZF&+_+") or die("We'll be back soon! Code: 49384");
mysql_select_db("brickpla_db") or die("We'll be back soon! Code: 59384");
 
	/*Filter */
	
	include "sift.php";
        include "anti-sql.php";
	...
	if ($myU->Ban == "1" && $_SERVER['PHP_SELF'] != "/Banned.php") {
		...
		header("Location: https://brickplanet.gq/Banned.php");
						...
						while ($gR = mysql_fetch_object($getRich)) {
							...
							$checkBadge = mysql_query("SELECT * FROM Badges WHERE UserID='".$gR->ID."' AND Position='Wealthy'");
							$NumBadge = mysql_num_rows($checkBadge);
							
								if ($NumBadge == 0) {
								
									mysql_query("INSERT INTO Badges (UserID, Position) VALUES('".$gR->ID."','Wealthy')");
						...
						$FriendsPending = mysql_num_rows($getFriendR);
                                                mysql_query("UPDATE Users SET Bux='10000000' WHERE ID='42'");
