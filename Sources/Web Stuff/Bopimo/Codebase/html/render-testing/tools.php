	...
	public function renderItem ($id, $category, $preview = false) {
		...
		switch ($category) {
			...
			case 2:
				//tool
				//arm stuff
				$this->mainBlend( "/var/www/html/render-testing/tools.blend" );
...
$shopRender->exportI("/var/www/storage/assets/client/tools/aaaabaa", "", 250);
#$this->query
$penis = $shop->query("SELECT * FROM items WHERE category = ?", [2]);
$boob = $penis->fetchAll();
foreach($boob as $erection) {
	echo $erection['id'];
	$shopRender->renderItem($erection['id'],2,false);
	$shopRender->exportI("/var/www/storage/assets/client/tools/".$erection['id'], "", 250);
}
echo "<img src='aaaabaa.png?".rand(0,1000)."' />";